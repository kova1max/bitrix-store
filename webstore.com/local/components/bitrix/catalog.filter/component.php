...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	switch ($field_code)
	{
		...
		case "SHOW_COUNTER":
			...
			break;
		case "SECTION_ID":
			$fsection = array_values($arResult["arrSection"]);
			$fid = array_keys($arResult["arrSection"]);

			unset($field_res);

			$i = 0;
			foreach($fsection as $id=>$sect) {
				$field_res[$i]['SECTION'] = trim(str_replace('.', '', $sect));
				$i++;
			}
			$i = 0;
			foreach($fid as $id=>$sect) {
				$field_res[$i]['ID'] = $sect;
				$i++;
			}
